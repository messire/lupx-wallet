...
- [x] Обновить [UI kit](design/ui-kit.md), [токены](design/tokens.css) и [витрину](design/preview.html): единая спокойная система поверхностей без декоративного glow. Добавить уточнение к ADR-0014.
...
- [x] ADR, фиксирующий принятие UI kit v0.2 как визуальной архитектуры фронтенда — [ADR-0014](architecture/adr/0014-frontend-visual-architecture-ui-kit-v0.2.md).
...
- M8 (code-quality-reviewer, решение координатора): после завершения переноса всех экранов `--color-*`/`--radius-*` alias-слой в `styles.scss` не имел ни одного реального потребителя (только мёртвый `UnderConstructionComponent`, не подключённый ни к одному маршруту) — удалены оба, как и предписывал ADR-0014.
...
- `ChipComponent` удалён — ни одного экрана с removable-фильтрами нет, держать неиспользуемый примитив под гипотетическое будущее — не наш случай (YAGNI, ADR-0014 не требует полноты набора компонентов).
