...
Статус на 2026-09-03 (продолжение). Обновляется по мере продвижения — при завершении каждого этапа/среза дополните таблицу и раздел "Следующий шаг" ниже.
...
| **Срез: ExchangeRates + Reporting + Audit + завершение Wallets** | Три новых backend-модуля (ExchangeRates — интеграция с Frankfurter, ADR-0001/ADR-0011; Reporting — Total Amount текущая/историческая, решение по риску №6 исключённых кошельков; Audit — сквозной подписчик на события всех модулей, ADR-0009/ADR-0010/ADR-0011), завершён жизненный цикл Wallets (UC-02…UC-06), весь оставшийся Angular UI (Operations/Transfers, справочники, BalanceHistory, ExchangeRates, Reporting, Audit, действия над кошельком), закрыты кросс-модульные TODO по ADR-0009 (случаи a/c реализованы, случай b — закрыт без порта, решение пользователя). Код-ревью и UX-ревью проведены, находки устранены. Подробности — см. раздел «ПЛАН: доведение приложения до полного соответствия архитектуре и требованиям» и записи по волнам ниже | *(следующий коммит)* |

## Следующий шаг

**Angular UI для Operations и BalanceHistory** — список операций по кошельку, форма создания дохода/расхода/корректировки, экран переводов, график/таблица истории баланса. После него — ExchangeRates, Audit, Reporting (бэкенд).
