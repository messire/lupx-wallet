using LupexWallet.ExchangeRates.Domain;
using LupexWallet.SharedKernel;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata.Builders;

namespace LupexWallet.ExchangeRates.Infrastructure;

/// <summary>Maps ExchangeRateQuote to exchange_rates.latest_exchange_rates (schema.md).</summary>
public sealed class ExchangeRateQuoteConfiguration : IEntityTypeConfiguration<ExchangeRateQuote>
{
    public void Configure(EntityTypeBuilder<ExchangeRateQuote> builder)
    {
        builder.ToTable("latest_exchange_rates", table =>
        {
            table.HasCheckConstraint("ck_latest_rates_distinct_currencies", "from_currency_id <> to_currency_id");
            table.HasCheckConstraint("ck_latest_rates_positive", "rate > 0");
        });

        builder.HasKey(x => new { x.FromCurrencyId, x.ToCurrencyId });

        builder.Property(x => x.FromCurrencyId)
            .HasConversion(id => id.Value, value => new CurrencyId(value))
            .HasColumnName("from_currency_id")
            .IsRequired();

        builder.Property(x => x.ToCurrencyId)
            .HasConversion(id => id.Value, value => new CurrencyId(value))
            .HasColumnName("to_currency_id")
            .IsRequired();

        builder.Property<decimal>("_rate").HasColumnName("rate").HasColumnType("numeric").IsRequired();

        builder.Property(x => x.FetchedAt).HasColumnName("fetched_at").IsRequired();
    }
}
