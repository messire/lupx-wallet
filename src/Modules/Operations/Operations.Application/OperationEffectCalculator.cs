...
/// <summary>
/// Вычисляет "предполагаемую" (intended) знаковую дельту, которую операция должна
/// вносить в баланс кошелька, и приростную (incremental) дельту, которую нужно реально
/// применить через IWalletBalanceGateway с учётом уже применённого ранее вклада той же
/// операции (Operation.AppliedDelta) — единая формула, корректно работающая для
/// создания, редактирования и удаления (см. комментарий в Operations.Domain.Operation).
///
/// Известное ограничение (см. ADR-0003): для Adjustment/Absolute "baseline" считается
/// от текущего CurrentBalance кошелька, а не от полной пересчитанной истории — это
/// корректно для текущего баланса, но не заменяет полноценный пересчёт истории,
/// который появится вместе с модулем BalanceHistory.
/// </summary>
public static class OperationEffectCalculator
{
    ...
    /// <summary>baseline = текущий баланс минус то, что уже внесла эта же операция ранее (0 при создании).</summary>
