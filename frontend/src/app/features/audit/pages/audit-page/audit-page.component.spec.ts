...
import { AuditComponent } from './audit-page.component';
...
describe('AuditComponent', () => {
  ...
  it('shows a prompt before any search is made', () => {
    ...
    expect(compiled.querySelector('app-empty-state')?.textContent).toContain('Выберите тип записи');
  });
  ...
  it('shows an explicit empty state (not an error) when history is empty', () => {
    ...
    const compiled = fixture.nativeElement as HTMLElement;
    expect(compiled.querySelector('app-empty-state')?.textContent).toContain('Изменений не найдено');
